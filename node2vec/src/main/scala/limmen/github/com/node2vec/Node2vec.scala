...
object Node2vec extends Serializable {
  ...
  def load(): this.type = {
    ...
    indexedNodes = inputTriplets.flatMap {
      case (srcId, dstId, weight) =>
  ...
  def indexingGraph(rawTripletPath: String): RDD[(Long, Long, Double)] = {
    ...
    rawEdges.map {
      ...
      case (src, (edge: (String, Double), srcIndex: Long)) =>
        try {
    ...
    }.filter(_ != null).join(node2id).map {
      case (dst, (edge: (Long, Double), dstIndex: Long)) =>
        try {
  ...
  def createNode2Id[T <: Any](triplets: RDD[(String, String, T)]) = triplets.flatMap {
    case (src, dst, weight) =>
