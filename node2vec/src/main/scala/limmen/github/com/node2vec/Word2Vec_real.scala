...
class Word2Vec extends Serializable {
    ...
    for (k <- 1 to numIterations) {
      ...
      val partial = newSentences.mapPartitionsWithIndex {
        case (idx, iter) =>
          ...
          println(s"Starting Iteration ${k} on partition ${idx}")
            ...
            case ((syn0, syn1, lastWordCount, wordCount), sentence) =>
              ...
              if (wordCount - lastWordCount > 10000) {
                ...
                if (verbose) {
                  ...
                  println(s"wordCount = ${wordCount + numWordsProcessedInPreviousIterations}, " + s"alpha1 = $alpha")
                  println(s"avg loss: ${(loss + globalLoss) / (numWordsProcessedInPreviousIterations + wordCount)}")
                }
              }
      ...
      globalLoss += incrLoss
      println(s"avg loss: ${globalLoss / (numWordsProcessedInPreviousIterations + trainWordsCount)}")
  ...
  class Word2VecModel(val wordIndex: Map[String, Int], val rawWordVectors: Array[Float]) {
    ...
    val wordVectors: Map[String, Array[Float]] = convertRawVectorsToWordVectors(rawWordVectors)
